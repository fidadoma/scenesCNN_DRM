...
p
ggsave(file.path(plots_dir, "Fig_chp4_results_accuracy.eps"), p, width = 6, height = 6)
...
p <- df_rt %>% 
  ...
  xlab("Type")
p
ggsave(file.path(plots_dir, "Fig_chp4_results_rt.eps"), p, width = 6, height = 6)
```
...
ggsave(file.path(plots_dir, "Fig_chp4_results_perhalves.eps"), p, width = 6, height = 6)
```
...
anova(lm1_justtype,lmnull)
av1 <- anova(lm1_justpayment,lmnull)
av1
av1$Chisq
...
glmer(correct~type*payment*part+(1+type|subject_id) + (1|category), df_acc2, family = binomial()) %>% summary()
...
p1 <- df_oldclose %>% ggplot(aes(x = FAs, y = Hs, group = as.factor(subject_id))) + 
  ...
  ggtitle("Target-close distractor discrimination")
...
p2 <- df_oldfar %>% ggplot(aes(x = FAs, y = Hs, group = as.factor(subject_id))) + 
  ...
  ggtitle("Target-far distractor discrimination")
...
p_SDT_model <- df_plot %>% ggplot(aes(x = x, y = y, col = type)) + geom_path(size = 1.2) + geom_vline(xintercept = 0) +
  theme(aspect.ratio = 1)
p_SDT_model
ggsave(file.path(plots_dir, "Fig_chp4_results_dprimedistance.eps"), p_dprimes, width = 6, height = 6)
