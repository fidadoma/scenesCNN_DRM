...
## Divide distractors by distance to targets - mean
...
df2_tgt <- df2_tgt %>% ungroup() %>% distinct()
df2_tgt_modeldist_mean <- df2_tgt %>% 
  filter(layer == "dist_to_tgts") %>% 
  group_by(subject_id,grp_order) %>% 
  do(divide_into_groups_tgt_dist_mean(.))

df2_tgt_modeldist_mean %>% xtabs(~type+model_type,.) %>% prop.table(1)
df2_tgt_modeldist_mean %>% filter(type!="target") %>% xtabs(~type+model_type,.) %>% psych::phi()
df2_tgt_modeldist_mean %>% filter(type != "target") %>% 
  group_by(model_type) %>% 
  summarize(correct = mean(correct))
...
df2_modelmem1 <- 
  df2_modelmem %>% 
    ungroup() %>% 
    select(subject_id, trial_id, category_centroid_type = type, memorability_model =model_type, correct)

df2_modeldisttoproto31 <- 
  df2_modeldisttoproto3  %>% 
    ungroup() %>% 
    select(subject_id, trial_id, dist_to_proto3 =model_type, correct) 

df2_modeldisttoproto51 <- 
  df2_modeldisttoproto5  %>% 
    ungroup() %>% 
    select(subject_id, trial_id, dist_to_proto5 =model_type, correct) 
    
df2_tgt_modeldist1 <- 
  df2_tgt_modeldist  %>% 
    ungroup() %>% 
    select(subject_id, trial_id, min_dist_to_tgt =model_type, correct) 
df_models_res <- df2_modelmem1 %>% 
  left_join(df2_modeldisttoproto31, by = c("subject_id", "trial_id", "correct")) %>% 
  left_join(df2_modeldisttoproto51, by = c("subject_id", "trial_id", "correct")) %>% 
  left_join(df2_tgt_modeldist1, by = c("subject_id", "trial_id", "correct")) %>% 
...
XXtab <- df_models_res_longer %>% 
  group_by(name,value) %>% 
  summarize(FA = sprintf("%.2f (%.2f)", mean(1-correct),sd(1-correct)))

XXtab


lm_orig_model <- df_models_res %>% lm(correct~category_centroid_type,.)
lm_dist_to_tgt <- df_models_res %>% lm(correct~min_dist_to_tgt,.)
lm_dist_to_proto_conv5 <- df_models_res %>% lm(correct~dist_to_proto5,.)
lm_dist_to_proto_conv3 <- df_models_res %>% lm(correct~dist_to_proto3,.)
lm_memorability <- df_models_res %>% lm(correct~memorability_model,.)
models_list <- list(lm_orig_model,lm_dist_to_tgt,lm_dist_to_proto_conv5,lm_dist_to_proto_conv3,lm_memorability)
models_list %>% 
  map(summary) %>%
  map_dbl("r.squared") %>% 
  round(2)
...
df_cont_entropy %>% 
  ggplot(aes(x = category, y = correct, col = type, group = type)) + 
  stat_summary(fun.data = "mean_cl_boot") + 
  stat_summary(fun.y = "mean", geom = "line") + 
