...
df <- df %>% anti_join(wrong_trials, by = c("prot_id_version"="prot_id","trial_id"))

participants <- readxl::read_excel(here("data","oddoneout","oddoneout_participants.xlsx"))

metrics <- readRDS(file = here("data", "oddoneout", "metrics_190611.rds"))
```
...
p <- metrics %>% 
  ...
  ggplot(aes(x = quintile, y = value, col = metric, group = metric)) + 
  stat_summary(fun.data = "mean_cl_boot") + 
  ylim(0,1)+
  theme(aspect.ratio = 1) +
  geom_hline(yintercept = 1/9) +
...
p
ggsave(file.path(plots_dir, "Fig_chp3_metrics.eps"), p, width = 6, height = 6)
