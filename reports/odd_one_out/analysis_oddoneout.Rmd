...
metrics_wide <- readRDS(file = here("data", "oddoneout", "metrics_wide_200317.rds"))
...
p
ggsave(file.path(plots_dir, "Fig_chp3_results.svg"), p, width = 6, height = 6)
...
p <- metrics %>% 
  ...
  stat_summary(fun.data = "mean_cl_boot", position = position_jitter(0.2)) + 
  theme(aspect.ratio = 1) +
  coord_cartesian(ylim = c(0,0.5)) +
  geom_hline(yintercept = 1/9) +
  ylab("Perc. correct") +
  scale_x_continuous("Quintile", breaks = c(2,3,4), labels = c("2","3","4")) +
  theme(text = element_text(size=18)) + scale_color_manual(labels = c("GIST", "HOG", "RGB", "SIFT"), values = c("#E41A1C","#377EB8","#4DAF4A","#984EA3")) + 
...
p <- metrics %>% 
  filter(metric!= "ssim_correct", metric != "metrics_combined_correct", metric != "metrics_max_correct") %>% 
  ...
  ylab("Perc. correct") +
  scale_x_continuous("Quintile", breaks = c(2,3,4), labels = c("2","3","4")) +
scale_color_manual(labels = c("Humans", "GIST", "HOG", "RGB", "SIFT"), values = c("#FF7F00","#E41A1C","#377EB8","#4DAF4A","#984EA3")) + 
  stat_summary(fun.y=mean, geom="line") +
...
x <- metrics %>% filter(!is.infinite(value_partic)) %>% select(value_partic, subject_id,quintile,metric)

x[complete.cases(x),] %>% 
ez::ezANOVA(dv = value_partic, 
              wid = subject_id,
              within=.(quintile,metric))

x <- metrics %>% filter(!is.infinite(value_fc7)) %>% select(value_fc7, subject_id,quintile,metric,category)

x[complete.cases(x),] %>% 
ez::ezANOVA(dv = value_fc7, 
              wid = subject_id,
              within=.(quintile,metric))
