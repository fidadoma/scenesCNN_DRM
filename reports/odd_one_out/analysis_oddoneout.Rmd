...
p <- df %>% ggplot(aes(x = quintile, y = correct)) + 
  ...
  ylab("Správnost odpovědí") + xlab("Kvintil")
p
ggsave(file.path(plots_dir, "Fig_chp3_.eps"), p, width = 6, height = 6)
...
df_for_onettest %>% group_by(quintile) %>% do(d=lsr::cohensD(.$correct, mu = 1/9)) %>% mutate(d = unlist(d))
...
glm2 <- glmer(correct ~ d_ratio + (1+d_ratio|subject_id) + (1+d_ratio|category), df, family = binomial())
glm2_null <- glmer(correct ~ (1+d_ratio|subject_id) + (1+d_ratio|category), df, family = binomial())
